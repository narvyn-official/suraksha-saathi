...
export async function GET() {
  try {
    ...
    const choices = await db().prepare("SELECT m.owner,m.role,m.user_id,c.name FROM team_members m LEFT JOIN training_centres c ON c.owner=m.owner WHERE m.active=1 AND m.user_id=?").bind(user.userId).all();
...
export async function POST(request: Request) {
  try {
    ...
    if(typeof input.invitation==="string" && input.invitation.length<500) {
      ...
      const row=await db().prepare("SELECT role FROM team_members WHERE owner=? AND email=? AND active=1 AND user_id IS NULL AND invite_hash=? AND invite_expires_at>?").bind(who,user.email.toLowerCase(),hash,Date.now()).first<{role:"admin"|"trainer"|"viewer"}>();
