...
export default function Login({mode="login"}:{mode?:Mode}) {
 ...
 <section className="account-card" aria-labelledby="account-title"><div className="account-intro"><span className="eyebrow">YOUR WORKSPACE</span><h1 id="account-title">{heading}</h1><p>{forgot?'Enter your account email to request a private reset link.':reset?'Use at least 15 characters. Resetting your password signs out all existing sessions.':signup?'Request an approved training centre or join your team with a private invitation. Creating an account does not grant admin access.':'Sign in to manage your team’s training.'}</p></div>
