...
assert.equal((await t('credentials',{action:'approve',requestId:id,reason:'Trainer cannot approve'})).status,403);
assert.equal((await call(owner,'credentials',{action:'approve',requestId:id,reason:'Centre admin cannot sign'})).status,403);
...
assert.equal((await t('credentials',{action:'approve',requestId:id,reason:'Self approval after role change'})).status,403);
db.prepare('UPDATE team_members SET role=? WHERE owner=? AND user_id=?').run('trainer',owner.userId,trainer.userId);
const approvals=await Promise.all([c('credentials',{action:'approve',requestId:id,reason:'Independent answers and evidence reviewed by QA'}),c('credentials',{action:'approve',requestId:id,reason:'Concurrent duplicate review must not duplicate issuance'})]);assert.equal(approvals.filter(r=>r.status===200).length,1,JSON.stringify(approvals));const signed=approvals.find(r=>r.status===200)!.data;
...
assert.equal((await c('credentials',{action:'approve',requestId:pending.data.request.id,reason:'Later failure means reject approval'})).status,400);
...
assert.equal((await c('credentials',{action:'approve',requestId:pending2.data.request.id,reason:'Suspended centre must fail'})).status,403);assert.equal((await t('records')).status,403);
...
assert.equal((await c('credentials',{action:'approve',requestId:pending2.data.request.id,reason:'Revoked certifier must fail'})).status,403);
...
assert.equal((await c('credentials',{action:'approve',requestId:pending2.data.request.id,reason:'Stale session cannot approve'})).status,403);
