...
class SceneView(context:Context, private val moduleId:String, private val hi:Boolean, private val inspect:Boolean=true,private val compact:Boolean=false):View(context){
 ...
 private val names=when(moduleId){"fire"->listOf("Alarm call point" to "अलार्म बटन","Extinguisher" to "अग्निशामक","Emergency exit" to "आपात निकास");"gas"->listOf("Restricted boundary" to "प्रतिबंधित सीमा","Gas detector" to "गैस डिटेक्टर","Attendant" to "बाहर का सहायक");else->listOf("Machine guard" to "मशीन गार्ड","Energy isolator" to "ऊर्जा अलग करने वाला उपकरण","Personal lock" to "व्यक्तिगत ताला")}
 ...
 init{isClickable=inspect;isFocusable=inspect;contentDescription=if(hi)"काल्पनिक प्रशिक्षण दृश्य। उपकरण की जानकारी खोलें।" else "Illustrative training scene. Open equipment information.";background=context.shape(Palette.soft,18)}
...
class ScenePainter{
 ...
 fun draw(c:Canvas,w:Float,h:Float,module:String,hi:Boolean,inspect:Boolean){
  ...
  val labels=when(module){"fire"->if(hi)listOf("अलार्म","अग्निशामक","निकास")else listOf("ALARM","EXTINGUISHER","EXIT");"gas"->if(hi)listOf("सीमा","डिटेक्टर","सहायक")else listOf("BOUNDARY","DETECTOR","ATTENDANT");else->if(hi)listOf("गार्ड","अलगाव","ताला")else listOf("GUARD","ISOLATOR","LOCK")}
