...
class AdminActivity:Activity() {
    ...
    private val writable get()=session?.optJSONObject("current")!=null&&role in listOf("admin","trainer")
    private val canRevoke get()=role in listOf("admin","certifier")
    ...
    private fun login(){session=null;detailOpen=false;pageHistory.clear();page="login";screen(t("Training centre","प्रशिक्षण केंद्र"),false)
        ...
        val remember=CheckBox(this).apply{text=t("Keep signed in on this private device","इस निजी उपकरण पर प्रवेश बनाए रखें")};content.add(remember,top=8)
        content.add(action(t(if(signup)"Create account" else "Sign in",if(signup)"खाता बनाएँ" else "साइन इन")){
            ...
            task({api.call("/api/auth/${if(signup)"sign-up" else "sign-in"}/email",payload)}){response->password.text.clear();if(response.optBoolean("twoFactorRedirect"))mfaChallenge()else refresh()}
    ...
    private fun mfaChallenge(){form(t("Confirm sign-in","प्रवेश पुष्टि")){f->val code=field(f,t("Authenticator or recovery code","ऑथेंटिकेटर या रिकवरी कोड"));val backup=CheckBox(this).apply{text=t("Use recovery code","रिकवरी कोड उपयोग करें")};f.add(backup);f.add(action(t("Verify","जाँचें")){task({api.call("/api/auth/two-factor/"+(if(backup.isChecked)"verify-backup-code"else"verify-totp"),JSONObject().put("code",code.text.toString()))}){code.text.clear();refresh()}})}}
    ...
    private fun refresh(){if(!::root.isInitialized)screen(t("Training centre","प्रशिक्षण केंद्र"),false);task({
        val fresh=api.call("/api/admin/session");val result=JSONObject().put("session",fresh)
        if(!fresh.isNull("current")){result.put("manage",api.call("/api/admin/manage"));result.put("records",api.call("/api/records"))};result
    }){session=it.getJSONObject("session");management=it.optJSONObject("manage")?:JSONObject();records=it.optJSONObject("records")?:JSONObject();if(session!!.isNull("current"))accessPending()else show(if(page=="login")"overview" else page)}}
    private fun accessPending(){screen(t("Centre access","केंद्र प्रवेश"),false);val app=session?.optJSONObject("application");body.add(label(t("Centre approval required","केंद्र की स्वीकृति आवश्यक है"),24f,bold=true));body.add(label(t("Creating an account does not grant administration or certification authority. Offline training remains available.","खाता बनाने से प्रशासन या प्रमाणपत्र जारी करने का अधिकार नहीं मिलता। ऑफलाइन प्रशिक्षण उपलब्ध है।"),16f),top=14);body.add(label("${app?.optString("status")?:"not-requested"} · ${app?.optString("reason")?:""}",16f),top=14)
        if(app?.optString("status") in listOf("not-requested","rejected"))button(t("Request centre approval","केंद्र स्वीकृति माँगें")){form(t("Centre application","केंद्र आवेदन")){f->val name=field(f,t("Centre name","केंद्र नाम"));val site=field(f,t("Site or organisation","स्थल या संस्था"));f.add(action(t("Submit application","आवेदन भेजें")){task({api.call("/api/governance",JSONObject().put("action","request").put("name",name.text.toString()).put("site",site.text.toString()))}){refresh()}},top=16)}}
        button(t("Switch / join a centre","केंद्र बदलें / जुड़ें")){switchCentre()};button(t("Refresh access","प्रवेश ताज़ा करें")){refresh()};button(t("Account & security","खाता और सुरक्षा")){account()}
    }
    ...
    private fun show(id:String,remember:Boolean=true){if(busy)return;if(session?.isNull("current")==true){accessPending();return};if(remember&&id!=page&&page!="login")pageHistory.add(page);detailOpen=false;page=id;when(id){
        "overview"->overview();"workers"->workers();"records"->assessments();"more"->more();"assignments"->assignments();"credentials"->credentials();"certification"->certificationQueue();"team"->team();"settings"->settings();"audit"->activityLog();"rooms"->rooms();"verify"->verify();"curriculum"->curriculum();else->overview()
    }}
    ...
    private fun assessment(row:JSONObject){val p=row.getJSONObject("payload");val result=p.optJSONObject("result")?:JSONObject();val actions=mutableListOf(t("View decisions","निर्णय देखें"));if(writable&&p.optString("kind")=="assessment"&&result.optBoolean("passed"))actions.add(t("Request certification review","प्रमाणपत्र समीक्षा माँगें"));PageDialogBuilder(this).setTitle(row.optString("worker_name")).setMessage("${module(p.optString("moduleId"))}\n${result.optInt("score")}% · ${if(result.optBoolean("passed"))"Passed" else "Needs review"}").setItems(actions.toTypedArray()){_,i->if(i==0)decisionEvidence(p)else PageDialogBuilder(this).setTitle(t("Request certification review?","प्रमाणपत्र समीक्षा माँगें? ")).setMessage(t("A different authorized certifier must review the latest assessment. This is not proof of practical competence.","यह सफल सिमुलेशन मूल्यांकन दर्शाता है। यह व्यावहारिक योग्यता प्रमाणित नहीं करता।")).setNegativeButton("Cancel",null).setPositiveButton(t("Continue","आगे बढ़ें")){_,_->issue(row.getString("id"))}.show()}.show()}
    private fun issue(attemptId:String) {
        ...
        DatePickerDialog(this,{_,y,m,d->expiry.set(y,m,d,23,59,59);val expiresAt=expiry.timeInMillis
            val note=EditText(this).apply{hint=t("Evidence / training note (10+ characters)","प्रशिक्षण विवरण (कम से कम 10 अक्षर)");minLines=2};PageDialogBuilder(this).setTitle(t("Requested credential expiry","अनुरोधित वैधता")).setMessage(date(expiresAt)).setView(note).setNegativeButton("Cancel",null).setPositiveButton(t("Submit request","अनुरोध भेजें")){_,_->task({api.call("/api/credentials",JSONObject().put("action","request").put("attemptId",attemptId).put("expiresAt",expiresAt).put("note",note.text.toString()))}){refresh();info(t("Request submitted for independent review.","स्वतंत्र समीक्षा के लिए अनुरोध भेजा गया।"))}}.show()
    ...
    private fun certificationQueue(){task({api.call("/api/credentials")}){result->val items=result.getJSONArray("requests").objects();listing(t("Certification review","प्रमाणपत्र समीक्षा"),items.map{it.optString("worker_name")+" · "+module(it.optString("module_id")) to it.optString("status")}){i->val q=items[i];form(t("Review request","अनुरोध की समीक्षा")){f->f.add(label(q.optString("worker_name")+" · "+q.optString("status"),20f,bold=true));f.add(label(q.optString("request_note"),16f),top=12);f.add(label(q.optString("review_reason").takeUnless{it=="null"}?:"",14f),top=8);f.add(action(t("Review assessment answers","मूल्यांकन के उत्तर देखें")){task({api.call("/api/records?attemptId="+q.getString("attempt_id"))}){decisionEvidence(it.getJSONObject("payload"))}},top=12)
        if(role=="certifier"&&q.optString("status")=="pending"&&q.optString("requested_by")!=session!!.getJSONObject("user").getString("userId")){val checklist=CheckBox(this).apply{text=t("I reviewed the answers, latest evidence and pilot-only scope","मैंने उत्तर, नवीनतम प्रमाण और पायलट सीमा की समीक्षा की")};f.add(checklist,top=12);val reason=field(f,t("Review reason (10+ characters)","समीक्षा कारण (कम से कम 10 अक्षर)"));for((decision,en,hin)in listOf(Triple("needs-information","Request more information","और जानकारी माँगें"),Triple("reject","Reject request","अनुरोध अस्वीकार करें"),Triple("approve","Approve and issue pilot credential","स्वीकृत कर पायलट प्रमाणपत्र जारी करें")))f.add(action(t(en,hin)){if(decision=="approve"&&!checklist.isChecked)notice(t("Review required","समीक्षा ज़रूरी"),t("Complete the evidence checklist before approval.","स्वीकृति से पहले प्रमाण सूची पूरी करें।"))else noticeDecision(decision,q.getString("id"),reason.text.toString())},top=12)}else f.add(label(t("A different authorized certifier must decide this request.","अलग अधिकृत समीक्षक को निर्णय देना होगा।"),14f),top=12)
    }}}}
    private fun noticeDecision(action:String,id:String,reason:String){PageDialogBuilder(this).setTitle(t("Confirm review decision","समीक्षा निर्णय की पुष्टि करें")).setMessage(reason).setNegativeButton(t("Cancel","रद्द"),null).setPositiveButton(t("Confirm","पुष्टि करें")){_,_->task({api.call("/api/credentials",JSONObject().put("action",action).put("requestId",id).put("reason",reason).apply{if(action=="approve")put("rubric",JSONObject().put("evidenceReviewed",true).put("scopeConfirmed",true).put("latestAssessment",true).put("identityBasis","not-verified").put("practical","not-assessed"))})}){certificationQueue()}}.show()}
