...
class AdminActivity:Activity() {
    ...
    override fun onDestroy(){generation++;jobs.shutdownNow();super.onDestroy()}
    @Deprecated("Framework navigation") override fun onBackPressed(){if(busy)return;if(session==null||page=="overview")finish()else show("overview")}
    private fun screen(title:String,withNav:Boolean=session!=null) {
        ...
        top.addView(action("‹",role=ActionRole.NEUTRAL){onBackPressed()}.apply{contentDescription=t("Back to learning","सीखने पर वापस जाएँ")},LinearLayout.LayoutParams(dp(48),dp(48)))
    ...
    private fun field(parent:LinearLayout,title:String,value:String="",type:Int=InputType.TYPE_CLASS_TEXT):EditText {parent.add(label(title,14f,Palette.muted,true),top=12);return EditText(this).apply{setText(value);textSize=16f;inputType=type;isSingleLine=true;minHeight=dp(48);setPadding(dp(12),dp(8),dp(12),dp(8));background=shape(Palette.surface,12,Palette.line);contentDescription=title;tag="admin-field-$title";parent.add(this,top=6)}}
    ...
    private fun form(title:String,setup:(LinearLayout)->Unit){screen(title);val scroll=ScrollView(this).apply{isFillViewport=true};val content=column();scroll.addView(content);body.addView(scroll,LinearLayout.LayoutParams(-1,-1));setup(content)}
    private fun login(){session=null;page="login";screen(t("Training centre","प्रशिक्षण केंद्र"),false)
        val scroll=ScrollView(this).apply{isFillViewport=true};val content=column();scroll.addView(content);body.addView(scroll,LinearLayout.LayoutParams(-1,0,1f))
    ...
    private fun server(){val input=EditText(this).apply{setText(api.base);hint="https://training.example.org";inputType=InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_URI;isSingleLine=true};val dialog=AlertDialog.Builder(this).setTitle(t("Training centre server","प्रशिक्षण केंद्र सर्वर")).setMessage(t("Enter the service address provided by your administrator. Changing it signs out this device.","व्यवस्थापक द्वारा दिया गया सर्वर पता दर्ज करें। बदलने पर साइन आउट होगा।")).setView(input).setNegativeButton(t("Cancel","रद्द"),null).setPositiveButton(t("Save","सहेजें"),null).create();dialog.setOnShowListener{dialog.getButton(-1).setOnClickListener{try{api.configure(input.text.toString());session=null;dialog.dismiss();login()}catch(e:Exception){input.error=e.message}}};dialog.show()}
    ...
    private fun show(id:String){if(busy)return;page=id;when(id){
    ...
    private fun overview(){screen(t("Training overview","प्रशिक्षण सारांश"));
        ...
        parent.addView(ScrollView(this).apply{isFillViewport=true;addView(body)},position,LinearLayout.LayoutParams(-1,0,1f));val centre=session!!.getJSONObject("current");body.add(label(centre.optString("name"),24f,bold=true));body.add(label(listOf(centre.optString("site"),role).filter{it.isNotBlank()}.joinToString(" · "),14f,Palette.muted),top=6)
    ...
    private fun listing(title:String,items:List<Pair<String,String>>,click:(Int)->Unit){screen(title);if(items.isEmpty()){body.add(label(t("Nothing here yet","अभी कोई रिकॉर्ड नहीं"),22f,bold=true),top=24);body.add(label(t("Use the actions below to get started.","नीचे दिए कार्यों से शुरू करें।"),15f,Palette.muted),top=12);body.addView(View(this),LinearLayout.LayoutParams(1,0,1f))}else{val list=ListView(this).apply{dividerHeight=dp(8);divider=android.graphics.drawable.ColorDrawable(Palette.canvas);adapter=object:BaseAdapter(){override fun getCount()=items.size;override fun getItem(p:Int)=items[p];override fun getItemId(p:Int)=p.toLong();override fun getView(p:Int,v:View?,parent:android.view.ViewGroup?):View=card().apply{add(label(items[p].first,16f,bold=true));if(items[p].second.isNotBlank())add(label(items[p].second,14f,Palette.muted),top=5);contentDescription=items[p].first+". "+items[p].second}};setOnItemClickListener{_,_,position,_->click(position)}};body.addView(list,LinearLayout.LayoutParams(-1,0,1f))}}
    private fun workers(){val values=rows("workers");listing(t("Workers","कर्मचारी"),values.map{it.optString("name") to it.optString("sector")}){val worker=values[it];val choices=if(writable)arrayOf(t("Learning history","सीखने का इतिहास"),t("Edit profile","प्रोफाइल बदलें"))else arrayOf(t("Learning history","सीखने का इतिहास"));AlertDialog.Builder(this).setTitle(worker.optString("name")).setItems(choices){_,index->if(index==1)workerForm(worker)else workerHistory(worker)}.show()};if(writable)button(t("Add worker","कर्मचारी जोड़ें")){workerForm(null)}}
    ...
    private fun assessment(row:JSONObject){val p=row.getJSONObject("payload");val result=p.optJSONObject("result")?:JSONObject();val actions=mutableListOf(t("View decisions","निर्णय देखें"));if(writable&&p.optString("kind")=="assessment"&&result.optBoolean("passed"))actions.add(t("Issue pilot credential","पायलट प्रमाणपत्र जारी करें"));AlertDialog.Builder(this).setTitle(row.optString("worker_name")).setMessage("${module(p.optString("moduleId"))}\n${result.optInt("score")}% · ${if(result.optBoolean("passed"))"Passed" else "Needs review"}").setItems(actions.toTypedArray()){_,i->if(i==0)decisionEvidence(p)else AlertDialog.Builder(this).setTitle(t("Issue pilot credential?","पायलट प्रमाणपत्र जारी करें? ")).setMessage(t("This confirms a passed simulation assessment. It does not certify practical competence.","यह सफल सिमुलेशन मूल्यांकन दर्शाता है। यह व्यावहारिक योग्यता प्रमाणित नहीं करता।")).setNegativeButton("Cancel",null).setPositiveButton("Issue"){_,_->issue(row.getString("id"))}.show()}.show()}
    private fun issue(attemptId:String) {
        ...
        DatePickerDialog(this,{_,y,m,d->expiry.set(y,m,d,23,59,59);val expiresAt=expiry.timeInMillis
            AlertDialog.Builder(this).setTitle(t("Credential expiry","प्रमाणपत्र की वैधता")).setMessage(date(expiresAt)).setNegativeButton("Cancel",null).setPositiveButton("Issue"){_,_->task({api.call("/api/credentials",JSONObject().put("attemptId",attemptId).put("expiresAt",expiresAt))}){credential(it)}}.show()
    ...
    private fun team(){val values=rows("team");listing(t("Team access","टीम प्रवेश"),values.map{it.optString("email") to "${it.optString("role")} · ${if(it.optInt("active")==0)"Revoked" else if(it.isNull("user_id"))"Invited" else "Active"}"}){val member=values[it];AlertDialog.Builder(this).setTitle(member.optString("email")).setItems(arrayOf("Change role / resend invitation",if(member.optInt("active")==1)"Revoke access" else "Restore access")){_,i->if(i==0)invite(member)else save(JSONObject().put("action","member").put("email",member.optString("email")).put("role",member.optString("role")).put("active",member.optInt("active")==0))}.show()};button(t("Invite colleague","सहकर्मी को आमंत्रित करें")){invite(null)}}
    ...
    private fun detail(title:String,value:String){form(title){it.add(label(value,16f));it.add(action(t("Back","वापस"),role=ActionRole.NEUTRAL){show(page)},top=20)}}
