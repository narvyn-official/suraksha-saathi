...
class RecallActivity:Activity(){
 ...
 private fun render(){
  ...
  if(item==null){
   ...
   due.take(20).forEach{r->val c=card();c.add(label(r.module.local("title",hi),14f,Palette.blue),bottom=8);c.add(label(r.question.local("prompt",hi),18f,Palette.ink,true),bottom=12);c.add(action(t("Recall this decision","यह निर्णय याद करें")){selected=r.key;answer=null;render()});body.add(c,bottom=14)}
   ...
   body.add(label(item.question.local("prompt",hi),22f,Palette.ink,true),bottom=18)
   if(answer==null){
    ...
    feedback.add(label(if(correct)t("You recalled a safe response","आपने सुरक्षित प्रतिक्रिया याद की")else t("A useful decision to revisit","यह निर्णय फिर दोहराना उपयोगी है"),20f,Palette.ink,true),bottom=12)
