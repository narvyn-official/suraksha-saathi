...
This document describes the implemented v0.1 pilot. Earlier architecture documents describe the target system; they are not claims that every feature is shipped.

## Implemented

- Native Kotlin Android application, Android 10+ minimum, fixed light theme, English and Hindi text.
- Two bundled modules: fire response and gas/confined-space decisions. Each has learning content, guided practice and eight assessment decisions.
- Native ARCore camera rendering with plane placement and pose-anchored action cards; on-screen fallback for unsupported phones. These are decision stations, not animated equipment manipulation or live hazard detection.
...
- React trainer dashboard, D1 persistence, owner-scoped routes, import validation and score replay, decision review, pilot issuance, credential QR/download, signature checking and revocation.
...
5. Add the remaining three safety domains once the truncated brief is confirmed.
