...
Only the API 36 hardware-accelerated ARM64 emulator was connected for this increment. The final APK has not been installed or completed on a physical phone. Prior Samsung camera-feed-only failure is still unresolved as a validation result. Follow [device validation](ar-device-validation.md) for actual station placement and full camera missions. Following the user’s download request, 0.5.6 is published as a [pilot prerelease](https://github.com/narvyn-official/suraksha-saathi/releases/tag/v0.5.6-pilot) with the APK and checksum. It is not promoted as an AR-verified release; no new demo video is supplied.
