...
**Next software priority:** apply the new camera-image freshness and revision guards to the older scored AR decision-station path in ArActivity.kt. That path still uses its earlier render-time tracking gate and should be hardened before broader AR piloting. Keep assessment writes once-only, preserve guided feedback across recovery, and test stale queued answers against stage changes. Do this before expanding camera component practice to more models.
