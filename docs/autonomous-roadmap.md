...
**Next software priority:** improve the equipment’s visual contact with its training surface through a lightweight, clearly illustrative contact-shadow pass shared by the viewer and AR renderer. Inspect all five models at changed angles and verify rendered output, lifecycle recovery and draw-call/allocation cost. Keep original model geometry, part targets and assessment evidence stable. Avoid suggesting that a synthetic contact cue is a measured real-world shadow or that visual polish establishes practical competence. Evaluate an authored asset pipeline separately before promising photorealism.
