...
Current implementation milestone: v0.5.3 AR-recovery candidate. Public downloads remain v0.5.0-pilot; v0.5.1-pilot is held as a draft after user feedback on the AR experience. Five modules (fire, gas/confined spaces, machinery/isolation, PPE/exposure, emergency/reporting), 40 decisions, English/Hindi pilot text, native offline SQLite progress, ARCore anchors, detailed original 3D equipment with material lighting, local spaced retrieval review, staged component recognition, signed pilot credentials, and persistent trainer analytics are implemented. See build-status.md, validation.md and reference-benchmark.md for evidence and limitations. The local dashboard is http://localhost:5173/ when its development server is running.

## First work to pick up

**Current blocking priority (15 September): user reports AR does not work.** Physical Samsung testing confirmed a camera feed but no usable tracking/placement, with delayed ARCore sensor poses and severe/critical thermal throttling. The final 0.5.3 recovery candidate adds real tracking reasons, explicit 30fps capture, a valid-surface ring, gated placement, and thermal/session recovery. USB disconnected after a preliminary patch installed; the final 0.5.3 APK still needs installation and real-phone testing. Do not claim AR success or publish a verified release. See [ar-recovery-validation.md](ar-recovery-validation.md).

A separate open regression/robustness check remains: the screen fire sweep test repeatedly hit 150ms sample-gap rejection on this emulator, including a 720px diagnostic run; gas completed. Diagnostics observed 190–330ms UI sample gaps during sweep. Removing screenshots and asynchronous movement injection did not resolve it, and test-driver experiments were reverted. The instruction panel now retains its viewport budget when feedback changes. Profile owner-thread callback work versus rendering contention and preserve the freshness/continuity checks. This is an unresolved limitation, not a reason to manufacture frames or learning credit.
