...
**Next software priority:** provide a labeled native placement action for the camera component and AR decision views, using the existing tracked-plane hit-test path. Their current placement requires a coordinate tap, which the hidden/custom camera surface does not expose to assistive services. Preserve fresh-image, revision and visible-target rules for answers, explicit screen alternatives, and all assessment/learning attribution. Validate rejection paths without claiming positive physical camera tracking.
